...
struct ReviewRangeTests {
    ...
    func labelsARange() {
        // Explicit timestamps, because both notes sit on line 34 of one file: without a tiebreak
        // the order falls through to the id, and the label would depend on which UUID sorts first.
